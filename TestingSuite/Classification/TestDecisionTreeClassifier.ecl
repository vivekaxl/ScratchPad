...
EXPORT TestDecisionTreeClassifier(raw_dataset_name, repeats) := FUNCTIONMACRO
	...
	SHARED RunDecisionTreeClassfier(DATASET(DiscreteField) trainIndepData, DATASET(DiscreteField) trainDepData, DATASET(DiscreteField) testIndepData, DATASET(DiscreteField) testDepData,t_Count Depth, REAL Purity=1.0) := FUNCTION
	...
	EXPORT WrapperRunDecisionTreeClassfier(DATASET(RECORDOF(AnyDataSet)) AnyDataSet):= FUNCTION
